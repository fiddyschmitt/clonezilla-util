...
namespace libGZip.Vendored.SharpZipLib.Zip.Compression.Streams
{
	/// <summary>
	...
	public class StreamManipulator
	{
		...
		/// Preloads the bit buffer with bits split off the byte before a mid-stream resume point -
		/// the equivalent of zlib's inflatePrime(), needed to resume DEFLATE decompression at a
		/// gztool/zran access point whose bit offset is not byte-aligned. The low
		/// <paramref name="bitCount"/> bits of <paramref name="value"/> become the next bits consumed.
		/// Only valid on a fresh or freshly-Reset() manipulator.
		/// </summary>
		public void PrimeBits(int bitCount, int value)
		{
			if (bitCount < 1 || bitCount > 7)
			{
				throw new ArgumentOutOfRangeException(nameof(bitCount));
			}
			if (bitsInBuffer_ != 0 || windowStart_ != windowEnd_)
			{
				throw new InvalidOperationException("PrimeBits requires an empty bit buffer");
			}
			buffer_ = (uint)(value & ((1 << bitCount) - 1));
			bitsInBuffer_ = bitCount;
		}
